...
```docker compose up --build```. Эта команда начнет собирать образы и создавать контейнеры для каждого сервиса. Если не хотите видеть логи сборки и работы контейнеров введите ```docker compose up --build -d``` ( флаг -d для фоновой работы )
Перейдите на адрес http://localhost или http://127.0.0.1 , без указания портов!! Проект готов к использованию!!!
